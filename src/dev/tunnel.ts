...
import { getFileExtension, base64ArrayBuffer, b64ToBuf, ab2str, spawnAsync, warn, logError, isBohrPath, getGlobalBohrPath, cliFetch } from '../utils'
...
export class Tunnel extends EventEmitter {
  ...
  async start() {
    ...
    let command = `cd ${getGlobalBohrPath()} ` + (isBohrPath() ? `&& node saveEnv.js && node saveWrangler.js ` : '') + `&& cd tunnel && npx --yes wrangler@beta -c wrangler.dev.toml dev --port ${this.port}`;
  ...
  async processRequest(ws: WebSocket, data: any) {
    ...
    try {
      const response = await cliFetch(url.toString(), fetchOptions);
